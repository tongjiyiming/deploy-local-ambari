...
NODE_NAME_PREFIX="tongjiyiming-ambari"
let N=3
let PORT=8080
let S=0
...
docker build -t tongjiyiming-ambari .
docker network create tongjiyiming 2> /dev/null
...
for i in $(seq $((N-1)));
do
    container_id=$(docker run -d --net tongjiyiming --memory-reservation 12GB --name $NODE_NAME_PREFIX-$i tongjiyiming-ambari)
